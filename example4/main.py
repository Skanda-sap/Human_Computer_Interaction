...
class SignUpApp(App):
    def build(self):
        ...
        title_label = Label(text='Create an Account', font_size='24sp', color=(0, 0, 0, 1))
        ...
        username_input = TextInput(hint_text='Username', size_hint_y=None, height=40, multiline=False)
        password_input = TextInput(hint_text='Password', password=True, size_hint_y=None, height=40, multiline=False)
        email_input = TextInput(hint_text='Email', size_hint_y=None, height=40, multiline=False)

        # Set text color to black for input fields
        username_input.foreground_color = (0, 0, 0, 1)
        password_input.foreground_color = (0, 0, 0, 1)
        email_input.foreground_color = (0, 0, 0, 1)
        ...
        def sign_up_button_callback(instance):
            ...
            message_label.text = f'Signing up with: Username: {username}, Email: {email}'

        # RGB values for CornflowerBlue (#6495ED)
        cornflower_blue_rgb = (100 / 255.0, 149 / 255.0, 237 / 255.0, 1.0)

        # Create sign-up button
        sign_up_button = Button(text='Sign Up', size_hint_y=None, height=40, on_press=sign_up_button_callback)
